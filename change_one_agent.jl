...
function change_one_agent(task_to_agent, x, r, b)
    neighborhood = Vector{Tuple{Int64, Int64}}()
    for task in 1:size(task_to_agent)[1]
        for agent in 1:size(x)[1] 
            if agent == task_to_agent[task]
                continue
            end
...
function update_sol_change_one_agent(x, task_to_agent, best_neighbor)
    agent_before = task_to_agent[best_neighbor[2]]
...
function LS_change_one_agent(task_to_agent, x, r, b, m, c, best_cost, stop)
    if !stop
        return stop, best_cost, x, task_to_agent
    end
    best_delta = 0
    best_neighbor = nothing
    neighborhood = change_one_agent(task_to_agent, x, r, b)
    if size(neighborhood)[1] != 0
        for (agent, task) in neighborhood
            agent_before = task_to_agent[task]
    ...
    if !stop
        ...
        x, task_to_agent = update_sol_change_one_agent(x, task_to_agent, best_neighbor)
    end
    return stop, best_cost, x, task_to_agent
end